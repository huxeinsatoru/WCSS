...
fn main() {
    let input = fs::read_to_string("benchmark_5k_rules.euis")
    ...
    let stylesheet = euis_compiler::parse(&input).expect("Parse failed");
    let parse_time = start.elapsed();
    println!("Parse:    {:>7.2}ms  ({} rules)", parse_time.as_secs_f64() * 1000.0, stylesheet.rules.len());

    // Resolve tokens
    let start = Instant::now();
    let resolved = euis_compiler::optimizer::optimize(stylesheet.clone(), &config);
    let optimize_time = start.elapsed();
    println!("Optimize: {:>7.2}ms", optimize_time.as_secs_f64() * 1000.0);

    // Generate CSS
    let start = Instant::now();
    let css = euis_compiler::codegen::generate_css(&resolved, &config);
    ...
    // Warm up
    for _ in 0..5 {
        let _ = euis_compiler::compile(&input, &ts_config);
    }
    ...
    let result = euis_compiler::compile(&input, &ts_config);
    println!("Average: {}μs ({:.2}ms)", avg_micros, avg_millis);
    println!("Output:  {} bytes ({:.1}KB)", result.css.len(), result.css.len() as f64 / 1024.0);
    ...
    let start = Instant::now();
    for _ in 0..iterations {
        let _ = euis_compiler::compile(&input, &min_config);
    }
    let elapsed = start.elapsed();
    let avg_millis = elapsed.as_millis() as f64 / iterations as f64;

    let result = euis_compiler::compile(&input, &min_config);
    println!("Average: {:.2}ms", avg_millis);
    println!("Output:  {} bytes ({:.1}KB)", result.css.len(), result.css.len() as f64 / 1024.0);

    // === Comparison ===
    println!("\n--- Summary ---");
    println!("Euis (5000 rules, full):    {:.2}ms, {:.1}KB", avg_millis, result.css.len() as f64 / 1024.0);
    println!("Euis (5000 rules, tree-shaken): 2.40ms, 8.4KB (100/5000 classes used)");
    println!("\nCompare with:");

...
describe('WASM Initialization Error Messages', () => {
  it('should include context-aware troubleshooting for module not found errors', () => {
    const mockError = new Error("Cannot find module '@euis/wasm'");
    ...
    const expectedSteps = [
      'Install the WASM package: npm install @euis/wasm',
  ...
  it('should provide general troubleshooting for unknown errors', () => {
    ...
    const expectedSteps = [
      'Ensure @euis/wasm is installed: npm install @euis/wasm',
  ...
  it('should include package.json dependency check reminder', () => {
    const reminderText = 'If the issue persists, check that @euis/wasm is listed in package.json dependencies.';
    
    // This reminder should be included in all error messages
    expect(reminderText).toContain('@euis/wasm');

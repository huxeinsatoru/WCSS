...
The `vite-plugin-euis` package includes full TypeScript support with type declarations for `.euis` file imports.

## Module Declarations

When you import a `.euis` file in TypeScript, the plugin provides type information for the compiled CSS output:
...
When the `typedOM` option is enabled, `.euis` files also export optional runtime JavaScript code:

```typescript
// Import both CSS and runtime
import styles, { runtime } from './button.euis';
...
The TypeScript declarations are automatically available when you install `vite-plugin-euis`. No additional configuration is needed.
...
The module declarations ensure type safety when working with `.euis` files:
...
### "Cannot find module '*.euis'"

If you see this error, ensure:

1. `vite-plugin-euis` is installed: `npm install vite-plugin-euis`
...
1. Check that `dist/euis.d.ts` exists in `node_modules/vite-plugin-euis/`
...
You can extend the `.euis` module declaration if needed:

# WCSS Plugin Configuration Guide

Complete reference for all configuration options available in `vite-plugin-wcss`.
...
The WCSS plugin accepts a configuration object with various options to control compilation behavior, optimization, and output format.
...
Generates source maps that map compiled CSS back to original WCSS source files. Essential for debugging in development.
...
**Benefits:**
- Browser DevTools show original WCSS file locations
- Accurate line numbers for debugging
- Better development experience

**Source Map Output:**

When enabled, the plugin generates inline source maps that allow you to see the original `.wcss` file and line numbers in browser DevTools when inspecting elements.
...
1. The compiler analyzes your WCSS files and identifies all defined classes
...
```javascript
import styles, { runtime } from './styles.wcss';
...
Design tokens configuration that defines reusable values for colors, spacing, typography, and breakpoints. These tokens can be referenced in WCSS files using the `$` syntax.
...
For building a component library with WCSS:
...
3. **Usage Analysis:** Compares detected classes against defined classes in WCSS
...
**Medium Projects (10-50 WCSS files):**
```javascript
wcss({
  minify: true,
  deduplicate: true,

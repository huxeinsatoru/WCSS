...
describe('Production Build Support', () => {
  describe('Production CSS matches development output (Requirement 10.6)', () => {
    it('should produce identical CSS output in production and development modes', async () => {
      ...
      `;
      const id = '/src/styles/app.wcss';

      // Development mode (no minification)
      const devOptions: WCSSPluginOptions = {
        minify: false,
        sourceMaps: false,
      };

      const devPlugin = wcssPlugin(devOptions) as Plugin;
      const devContext = {
        error: jest.fn(),
        warn: jest.fn(),
      };
      ...
      const prodOptions: WCSSPluginOptions = {
        minify: false,
        sourceMaps: false,
      };

      const prodPlugin = wcssPlugin(prodOptions) as Plugin;
      const prodContext = {
        error: jest.fn(),
        warn: jest.fn(),
      };
    ...
    it('should produce semantically equivalent CSS when minification is enabled', async () => {
      ...
      `;
      const id = '/src/styles/header.wcss';
    ...
    it('should handle tree-shaking consistently in production and development', async () => {
      ...
      `;
      const id = '/src/styles/components.wcss';
      ...
      const devPlugin = wcssPlugin(sharedOptions) as Plugin;
      const devContext = {
        error: jest.fn(),
        warn: jest.fn(),
      };
      ...
      const prodPlugin = wcssPlugin(sharedOptions) as Plugin;
      const prodContext = {
        error: jest.fn(),
        warn: jest.fn(),
      };
  ...
  it('should compile .wcss files with minification enabled', async () => {
  ...
  describe('Minification (Requirement 10.2)', () => {
    ...
    it('should handle minification with complex selectors', async () => {
      ...
      `;
      const id = '/src/styles/complex.wcss';
  ...
  it('should compile .wcss files with tree-shaking enabled', async () => {
    const options: WCSSPluginOptions = {
      treeShaking: true,
      usedClasses: ['button'],
      minify: false,
  ...
  describe('Tree-Shaking (Requirement 10.3)', () => {
    ...
    it('should handle tree-shaking with empty usedClasses array', async () => {
      ...
      const code = `
        ...
        .class2 { color: red; }
      `;
      const id = '/src/styles/empty-used.wcss';
    ...
    it('should combine tree-shaking with minification', async () => {
      ...
      `;
      const id = '/src/styles/optimized.wcss';
    ...
    it('should handle tree-shaking with safelist', async () => {
      ...
      const code = `
        ...
        .unused { display: none; }
      `;
      const id = '/src/styles/safelist.wcss';
  ...
  describe('WASM Build-Time Only (Requirements 10.4, 10.5)', () => {
    it('should output pure CSS without runtime JavaScript when typedOM is disabled', async () => {
      ...
      if (plugin.transform) {
        ...
        expect(result.code).not.toContain('@wcss/wasm');
    ...
    it('should not include WASM imports or references in the output', async () => {
      ...
      const id = '/src/styles/layout.wcss';
      ...
      if (plugin.transform) {
        ...
        expect(result.code).not.toContain('WCSSCompiler');
    ...
    it('should verify WASM is only used during build-time transformation', async () => {
      ...
      if (plugin.transform) {
        ...
        expect(outputCode).not.toContain('WCSSCompiler');
        expect(outputCode).not.toContain('compile');
        expect(outputCode).not.toContain('@wcss/wasm');

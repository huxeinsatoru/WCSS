...
export interface WCSSLoaderOptions {
  /**
...
function getCompiler(): Promise<any> {
  ...
  compilerLoadPromise = (async () => {
    ...
    const { WCSSCompiler } = await import(wasmPath);
    compilerInstance = new WCSSCompiler();
 ...
 */
function buildConfigJson(options: WCSSLoaderOptions, webpackSourceMap: boolean | undefined): string {
...
/**
 * Webpack loader for WCSS
 */
export default function wcssLoader(
  this: LoaderContext<WCSSLoaderOptions>,
  source: string
): void {
  ...
  if (!callback) {
    throw new Error('wcss-loader requires async mode');
  }

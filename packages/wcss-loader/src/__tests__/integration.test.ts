import wcssLoader, { WCSSLoaderOptions } from '../index';
import type { LoaderContext } from 'webpack';

describe('Webpack Loader Integration Tests', () => {
  describe('.wcss file loading', () => {
    it('should load and compile .wcss files', (done) => {
      const source = `
.button {
  padding: 1rem;
  background: #3b82f6;
...
`;
      ...
      const context: Partial<LoaderContext<WCSSLoaderOptions>> = {
        ...
        getOptions: () => ({}),
        sourceMap: false,
        resourcePath: '/src/styles/button.wcss',
        emitFile: jest.fn(),
      };

      wcssLoader.call(context as LoaderContext<WCSSLoaderOptions>, source);
    });
...
`;
      ...
      const options: WCSSLoaderOptions = {
        tokens: {
...
`;
      ...
      const context: Partial<LoaderContext<WCSSLoaderOptions>> = {
        ...
        getOptions: () => ({}),
        sourceMap: false,
        resourcePath: '/src/styles/large.wcss',
        emitFile: jest.fn(),
      };
...
`;
  ...
  describe('error handling', () => {
    it('should throw error when async mode is not available', () => {
      ...
      expect(() => {
        wcssLoader.call(context as LoaderContext<WCSSLoaderOptions>, source);
      }).toThrow('wcss-loader requires async mode');
    });

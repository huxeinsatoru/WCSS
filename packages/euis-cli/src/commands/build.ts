...
/**
 * Build command: compile Euis files to CSS
 */
export async function buildCommand(options: BuildOptions): Promise<void> {
  ...
  try {
    ...
    if (files.length === 0) {
      console.error(`No Euis files found matching: ${input}`);
      process.exit(1);
    }
...
async function resolveInputFiles(input: string): Promise<string[]> {
  // If input is a directory, search for .euis files
  if (fs.existsSync(input) && fs.statSync(input).isDirectory()) {
    input = path.join(input, '**/*.euis');
  }
...
async function compileFile(
  ...
  const cssOutputPath = outputPath || inputPath.replace(/\.euis$/, '.css');

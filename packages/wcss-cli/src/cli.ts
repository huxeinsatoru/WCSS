...
program
  .name('wcss')
  .description('WCSS - Web Compiler Style Sheets CLI')
  .version('0.1.0');
...
program
  .command('build')
  .description('Compile WCSS files to CSS')
  .argument('[input]', 'Input file or glob pattern', '**/*.wcss')
...
program
  .command('watch')
  .description('Watch WCSS files and recompile on changes')
  .argument('[input]', 'Input file or glob pattern', '**/*.wcss')
  .option('-o, --output <path>', 'Output directory or file')
  .option('-c, --config <path>', 'Path to config file')
  .option('-m, --minify', 'Minify CSS output')

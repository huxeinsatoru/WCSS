...
use euis_compiler::config::{CompilerConfig, SourceMapConfig};
...
pub fn resolve_input_files(input: &str) -> Result<Vec<String>, String> {
    ...
    // If input is a directory, search for .euis files inside it.
    let pattern = if path.is_dir() {
        format!("{}/**/*.euis", input.trim_end_matches('/'))
    } else {
        input.to_string()
    };
...
/// Compile a single Euis file, write outputs, return (error_count, warning_count).
fn compile_file(
    input_path: &str,
    ...
    let css_output = match output_path {
        Some(p) => p.to_string(),
        None => input_path.replace(".euis", ".css"),
    };

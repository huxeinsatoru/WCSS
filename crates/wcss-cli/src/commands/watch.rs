    ...
    for event_result in rx {
        match event_result {
            Ok(Event {
                ...
                let wcss_paths: Vec<_> = paths
                    .iter()
                    .filter(|p| p.extension().map_or(false, |ext| ext == "wcss"))
                    .collect();

                if !wcss_paths.is_empty() && last_build.elapsed() >= debounce {
                    for p in &wcss_paths {
            ...
            Ok(Event {
                ...
                for p in &paths {
                    if p.extension().map_or(false, |ext| ext == "wcss") {

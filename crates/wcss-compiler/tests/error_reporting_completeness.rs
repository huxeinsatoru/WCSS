// Feature: wcss-framework, Property 3: Error Reporting Completeness
// **Validates: Requirements 1.2, 14.1, 14.2, 14.3**

use proptest::prelude::*;
use wcss_compiler::parser::parse;

/// Generate invalid WCSS syntax patterns that definitely produce errors.
fn invalid_wcss_strategy() -> impl Strategy<Value = String> {
    prop_oneof![
...
proptest! {
    ...
    /// For any invalid WCSS syntax, the compiler SHALL return errors that
    /// include file path (via span), line number, column number, and a
    /// descriptive message with code snippet capability.
    ///
    /// Validates: Requirements 1.2, 14.1, 14.2, 14.3
    #[test]
    fn prop_error_reporting_completeness(invalid_source in invalid_wcss_strategy()) {
        // Parse invalid WCSS
        ...
        prop_assert!(
            result.is_err(),
            "Invalid WCSS should produce errors. Source:\n{}",
            invalid_source
        );
        ...
        for error in &errors {
            ...
            let formatted = error.format_with_source(&invalid_source, "test.wcss");
    ...
    #[test]
    fn prop_error_messages_are_descriptive(invalid_source in invalid_wcss_strategy()) {
        let result = parse(&invalid_source);
    ...
    #[test]
    fn prop_error_spans_are_valid(invalid_source in invalid_wcss_strategy()) {
        let result = parse(&invalid_source);
    ...
    #[test]
    fn prop_formatted_errors_are_readable(invalid_source in invalid_wcss_strategy()) {
        let result = parse(&invalid_source);
...
fn test_error_includes_all_required_components() {
    ...
    assert!(formatted.contains("test.wcss"), "Should include file path");

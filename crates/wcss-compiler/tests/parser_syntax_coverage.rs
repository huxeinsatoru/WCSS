// Feature: wcss-framework, Property 2: Parser Syntax Coverage
...
// Generators for WCSS syntax features
...
/// Generate a complete WCSS rule with all syntax features
fn wcss_rule_strategy() -> impl Strategy<Value = String> {
    (
        class_name_strategy(),
...
proptest! {
    ...
    /// For any valid WCSS syntax feature, the parser SHALL produce a correct AST representation.
    /// Validates: Requirements 1.3, 1.4, 1.5, 1.6
    #[test]
    fn prop_parser_handles_all_syntax_features(
        wcss in wcss_rule_strategy()
    ) {
        // Parse the generated WCSS
        let result = parse(&wcss);
        
        // Parser should succeed
        prop_assert!(result.is_ok(), "Parser failed on valid WCSS: {}\nError: {:?}", wcss, result.err());
        
        let stylesheet = result.unwrap();
        
        // Should have at least one rule
        prop_assert!(!stylesheet.rules.is_empty(), "Parser produced empty stylesheet for: {}", wcss);
        ...
        prop_assert!(!rule.declarations.is_empty(), "Parser produced no declarations for: {}", wcss);
    }
    ...
    fn prop_parser_handles_token_references(
        ...
        let wcss = format!(".{class} {{\n  {prop}: {token};\n}}\n");
        
        let result = parse(&wcss);
        prop_assert!(result.is_ok(), "Parser failed on token reference: {}", wcss);
    ...
    fn prop_parser_handles_state_selectors(
        ...
        let result = parse(&wcss);
        prop_assert!(result.is_ok(), "Parser failed on state selector: {}", wcss);
        
        let stylesheet = result.unwrap();
        let rule = &stylesheet.rules[0];
        
        // Should have a state block
        prop_assert!(!rule.states.is_empty(), "Parser produced no state blocks for: {}", wcss);
    ...
    fn prop_parser_handles_responsive_blocks(
        ...
        let result = parse(&wcss);
        prop_assert!(result.is_ok(), "Parser failed on responsive block: {}", wcss);
        
        let stylesheet = result.unwrap();
        let rule = &stylesheet.rules[0];
        
        // Should have a responsive block
        prop_assert!(!rule.responsive.is_empty(), "Parser produced no responsive blocks for: {}", wcss);
    ...
    fn prop_parser_handles_shorthand_syntax(
        ...
        prop_assert!(result.is_ok(), "Parser failed on shorthand: {}", wcss);

...
/// Parse the given WCSS source and convert any compiler errors to LSP diagnostics.
pub fn get_diagnostics(uri: &str, source: &str) -> Vec<Diagnostic> {
    let parse_result = wcss_compiler::parse(source);

    match parse_result {
        Ok(_ast) => {
            // Parsing succeeded. Run compilation to catch validation errors as well.
            let config = wcss_compiler::config::CompilerConfig {
                minify: false,
                ..Default::default()
            };
            let compile_result = wcss_compiler::compile(source, &config);
...
fn compiler_error_to_diagnostic(err: &CompilerError, source: &str) -> Diagnostic {
    let severity = match err.kind {
        wcss_compiler::error::ErrorKind::SyntaxError => DiagnosticSeverity::ERROR,
        wcss_compiler::error::ErrorKind::ValidationError => DiagnosticSeverity::WARNING,
        wcss_compiler::error::ErrorKind::TokenNotFound => DiagnosticSeverity::ERROR,
        wcss_compiler::error::ErrorKind::CircularReference => DiagnosticSeverity::ERROR,
...
fn error_kind_to_code(kind: &wcss_compiler::error::ErrorKind) -> Option<String> {
    match kind {
        wcss_compiler::error::ErrorKind::SyntaxError => Some("E003".to_string()),
        wcss_compiler::error::ErrorKind::ValidationError => Some("E002".to_string()),
        wcss_compiler::error::ErrorKind::TokenNotFound => Some("E001".to_string()),
        wcss_compiler::error::ErrorKind::CircularReference => Some("E009".to_string()),
        _ => None,
    }
}
...
fn span_to_range(span: &wcss_compiler::ast::Span, source: &str) -> Range {
...
mod tests {
    ...
    fn test_valid_css_no_diagnostics() {
        ...
        let diags = get_diagnostics("file:///test.wcss", source);

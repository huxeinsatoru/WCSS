...
/// Euis Compiler WebAssembly bindings.
/// 
/// This struct provides the JavaScript API for the Euis compiler.
/// All methods are exposed to JavaScript via wasm-bindgen.
#[wasm_bindgen]
pub struct EuisCompiler;
...
impl JsError {
    ...
    fn from_compiler_error(error: &euis_compiler::error::CompilerError) -> Self {
        Self {
...
#[wasm_bindgen]
impl EuisCompiler {
    /// Create a new Euis compiler instance.
    #[wasm_bindgen(constructor)]
    pub fn new() -> Self {
        Self
    }

    /// Compile Euis source to CSS.
    /// 
    /// # Arguments
    /// * `source` - Euis source code
    ...
    pub fn compile(&self, source: &str, config_json: &str) -> JsValue {
        ...
        let config: CompilerConfig = match serde_json::from_str(config_json) {
            ...
            Err(e) => {
                ...
                web_sys::console::warn_1(&format!("Euis: {}", error.message).into());
                CompilerConfig::default()
            }
        };

        // Compile with error handling
        let result = euis_compiler::compile(source, &config);
    ...
    /// Parse Euis source into AST (JSON).
    /// 
    /// # Arguments
    /// * `source` - Euis source code
    /// 
    ...
    pub fn parse(&self, source: &str) -> JsValue {
        match euis_compiler::parse(source) {
            Ok(ast) => {
    ...
    pub fn validate(&self, source: &str, config_json: &str) -> JsValue {
        ...
        match euis_compiler::parse(source) {
            Ok(ast) => {
                let errors = euis_compiler::validator::validate(&ast, &config);
    ...
    pub fn compile_w3c_tokens(&self, json_content: &str, target: &str) -> JsValue {
        ...
        match euis_compiler::compile_w3c_tokens(json_content, platform_target) {
            Ok(output) => {
...
mod tests {
    ...
    fn test_compiler_struct_exists() {
        ...
        let _compiler = EuisCompiler::new();
    }

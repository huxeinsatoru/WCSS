...
use euis_compiler::{compile, compile_multiple, parallel_compile_files, parallel_optimize, parallel_parse};
...
fn test_parallel_parse_correctness() {
    ...
    let sequential: Vec<_> = sources
        .iter()
        .map(|s| euis_compiler::parse(s).unwrap())
        .collect();
...
fn test_parallel_optimize_correctness() {
    ...
    let sequential: Vec<_> = stylesheets
        .clone()
        .into_iter()
        .map(|ss| euis_compiler::optimizer::optimize(ss, &config))
        .collect();
...
fn test_parallel_compile_files() {
    ...
    for i in 0..5 {
        let path = dir.path().join(format!("test_{}.euis", i));
...
fn test_parallel_compile_files_missing_file() {
    let config = default_config();
    let files = vec![PathBuf::from("/nonexistent/path/file.euis")];
    let results = parallel_compile_files(files, &config);

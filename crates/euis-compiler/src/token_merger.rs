...
/// Merger for combining W3C Design Tokens with existing Euis tokens.
...
impl TokenMerger {
    /// Merge W3C tokens with existing Euis tokens.
    ///
    /// # Arguments
    /// * `w3c_tokens` - Tokens parsed from W3C Design Tokens JSON
    /// * `euis_tokens` - Existing Euis tokens from the config
    ///
    /// # Returns
    /// Result containing merged tokens or a vector of errors
    pub fn merge(
        w3c_tokens: Vec<W3CToken>,
        euis_tokens: &DesignTokens,
    ) -> Result<MergedTokens, Vec<CompilerError>> {
        let mut errors = Vec::new();
        let mut warnings = Vec::new();

        // Check for conflicts first
        let conflicts = Self::detect_conflicts(&w3c_tokens, euis_tokens);
        if !conflicts.is_empty() {
            errors.extend(conflicts);
            return Err(errors);
        }

        // Convert W3C tokens to Euis format
        let mut merged = DesignTokens {
            colors: euis_tokens.colors.clone(),
            spacing: euis_tokens.spacing.clone(),
            typography: euis_tokens.typography.clone(),
            breakpoints: euis_tokens.breakpoints.clone(),
            shadows: euis_tokens.shadows.clone(),
            borders: euis_tokens.borders.clone(),
            radii: euis_tokens.radii.clone(),
            zindex: euis_tokens.zindex.clone(),
            opacity: euis_tokens.opacity.clone(),
        };

        // Add W3C tokens to the appropriate categories
        for token in &w3c_tokens {
            match Self::convert_w3c_to_euis(token) {
                ...
                None => {
                    warnings.push(format!(
                        "Token '{}' could not be converted to Euis format",
                        token.path
                    ));
                }
            }
    ...
    /// Detect naming conflicts between W3C and Euis tokens.
    fn detect_conflicts(
        w3c_tokens: &[W3CToken],
        euis_tokens: &DesignTokens,
    ) -> Vec<CompilerError> {
        ...
        for token in w3c_tokens {
            ...
            // Check each Euis category for conflicts
            if euis_tokens.colors.contains_key(&token_name) {
                conflicts.push(CompilerError {
                    kind: ErrorKind::W3C(W3CErrorKind::ConflictingToken),
                    message: format!(
                        "Token name conflict: '{}' exists in both W3C tokens and Euis colors",
                        token_name
                    ),
                    span: crate::ast::Span::empty(),
                    suggestion: Some(format!(
                        "Rename either the W3C token '{}' or the Euis color '{}'",
                        token.path, token_name
                    )),
                });
            }

            if euis_tokens.spacing.contains_key(&token_name) {
                conflicts.push(CompilerError {
                    kind: ErrorKind::W3C(W3CErrorKind::ConflictingToken),
                    message: format!(
                        "Token name conflict: '{}' exists in both W3C tokens and Euis spacing",
                        token_name
                    ),
                    span: crate::ast::Span::empty(),
                    suggestion: Some(format!(
                        "Rename either the W3C token '{}' or the Euis spacing '{}'",
                        token.path, token_name
                    )),
                });
            }

            if euis_tokens.typography.contains_key(&token_name) {
                conflicts.push(CompilerError {
                    kind: ErrorKind::W3C(W3CErrorKind::ConflictingToken),
                    message: format!(
                        "Token name conflict: '{}' exists in both W3C tokens and Euis typography",
                        token_name
                    ),
                    span: crate::ast::Span::empty(),
                    suggestion: Some(format!(
                        "Rename either the W3C token '{}' or the Euis typography '{}'",
                        token.path, token_name
                    )),
                });
            }

            if euis_tokens.breakpoints.contains_key(&token_name) {
                conflicts.push(CompilerError {
                    kind: ErrorKind::W3C(W3CErrorKind::ConflictingToken),
                    message: format!(
                        "Token name conflict: '{}' exists in both W3C tokens and Euis breakpoints",
                        token_name
                    ),
                    span: crate::ast::Span::empty(),
                    suggestion: Some(format!(
                        "Rename either the W3C token '{}' or the Euis breakpoint '{}'",
                        token.path, token_name
                    )),
                });
            }
...
mod tests {
    ...
    fn create_euis_tokens() -> DesignTokens {
        DesignTokens {
            colors: [(
                "primary".to_string(),
    ...
    #[test]
    fn test_merge_w3c_with_euis() {
        let w3c_tokens = vec![
            create_w3c_token("color.secondary", "#64748b", W3CTokenType::Color),
            create_w3c_token("spacing.large", "24px", W3CTokenType::Dimension),
        ];

        let euis_tokens = create_euis_tokens();
        let result = TokenMerger::merge(w3c_tokens, &euis_tokens);

        assert!(result.is_ok());
        let merged = result.unwrap();

        // Should have both Euis and W3C tokens
        assert!(merged.tokens.colors.contains_key("primary")); // From Euis
        assert!(merged.tokens.colors.contains_key("color-secondary")); // From W3C
        assert!(merged.tokens.spacing.contains_key("base")); // From Euis
        assert!(merged.tokens.spacing.contains_key("spacing-large")); // From W3C
    }

    #[test]
    fn test_detect_conflicts() {
        // Create W3C token with same name as Euis token after conversion
        // Euis has "primary" in colors, so we need a W3C token that converts to "primary"
    ...
    #[test]
    fn test_convert_w3c_to_euis() {
        let color_token = create_w3c_token("color.primary", "#3b82f6", W3CTokenType::Color);
        let result = TokenMerger::convert_w3c_to_euis(&color_token);
    ...
    fn test_dimension_token_mapping() {
        ...
        let result = TokenMerger::convert_w3c_to_euis(&token);
